...
def alg():
    ...
    for threshold in thresholds:
        for percent in chunk_percent:
            ...
            frequent_items = apriori(df, min_support=threshold, use_colnames=True)
            ...
            result_list = [int(threshold * 100),len(frequent_items), (end - start) * 1000,
                                                                    chunk_size, int(percent * 100)]
            result_df.loc[len(result_df)] = result_list

            print("%d %d %.3f %d" % (int(threshold * 100), len(frequent_items) , ((end - start) * 1000), chunk_size))
